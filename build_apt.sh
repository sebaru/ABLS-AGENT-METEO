#!/bin/bash
# build_apt.sh -- Build DEB package for abls-agent-meteo via CPack
set -euo pipefail
...
echo "Building DEB package for abls-agent-meteo..."
...
if [[ "$CLEAN" == "true" ]]; then
  rm -f "$BUILD_DIR"/abls-agent-meteo*.deb
  rm -f "$ARTIFACT_DIR"/abls-agent-meteo*.deb
fi
...
while IFS= read -r deb_file; do
  if command -v dpkg-deb >/dev/null 2>&1; then
    ...
    if [[ "$pkg_name" == "abls-agent-meteo" ]]; then
  ...
  case "$(basename "$deb_file")" in
    *runtime*.deb|abls-agent-meteo-*.deb)
      runtime_deb="$deb_file"
      ;;
  esac
